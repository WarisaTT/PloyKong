  ...
  <div class="builder-layout">
    ...
    <header class="builder-topbar">
      ...
      <div class="builder-mobile-tabs @768:flex">
        <button type="button" class="mobile-tab-btn" :class="{ active: !isLeftCollapsed }" @click="openSidebar('left')"
          title="Blocks">
          <Blocks :size="18" />
        </button>
        <button type="button" class="mobile-tab-btn" :class="{ active: !isRightCollapsed && !selectedSectionId }"
          @click="openSidebar('right')" title="Design">
          <Palette :size="18" />
        </button>
        <button type="button" v-if="selectedSectionId" class="mobile-tab-btn pulse-hint"
          :class="{ active: !isRightCollapsed && selectedSectionId }" @click="scrollToEditor" title="Edit Section">
          <Pencil :size="18" />
        </button>
        <button type="button" class="mobile-tab-btn" :disabled="!store.activePortfolio?.is_published"
          @click="copyPublicLink" title="Copy Link">
          <Check v-if="copied" :size="18" class="text-success" />
          <Copy v-else :size="18" />
        </button>
        <button type="button" v-if="!isLeftCollapsed || !isRightCollapsed" class="mobile-tab-btn hide-btn"
          @click="isLeftCollapsed = true; isRightCollapsed = true" title="Close Sidebar">
          <X :size="18" />
        </button>
      </div>

      <div class="topbar-actions">
        <button type="button" class="btn btn-secondary btn-sm btn-icon-only-mobile"
          :disabled="!store.activePortfolio?.is_published" @click="previewPortfolio">
          <Eye v-if="store.activePortfolio?.is_published" :size="14" />
          <EyeOff v-else :size="14" />
          <span class="mobile-hide">Preview</span>
        </button>
        <button type="button" class="btn btn-sm btn-icon-only-mobile publish-btn"
          :class="store.activePortfolio?.is_published ? 'btn-danger' : 'btn-primary'" @click="togglePublish">
    ...
    <Transition name="fade-backdrop">
      <div v-if="(!isLeftCollapsed || !isRightCollapsed) && isMobile" class="mobile-backdrop"
        @click="isLeftCollapsed = true; isRightCollapsed = true" />
    </Transition>

    <!-- ─── Main Builder Area ──────────────────────────────────────────── -->
    <div class="builder-main" :class="{
      'left-collapsed': isLeftCollapsed,
      'right-collapsed': isRightCollapsed,
    }" :style="dashboardThemeVars">
      <!-- Left: Block Palette -->
      <aside class="block-palette" :class="{ collapsed: isLeftCollapsed }" :style="{
        ...
        <div class="palette-header">
          ...
          <button type="button" class="mobile-close-sidebar @768:flex" @click="isLeftCollapsed = true">
            <X :size="18" />
          </button>
          <span class="palette-hint mobile-hide" :style="{ paddingTop: '10px' }">Click to add</span>
        </div>
        <div class="palette-list">
          <div v-for="block in BLOCK_TYPES" :key="block.type" class="palette-item" @click="addBlock(block.type)">
      ...
      <div class="canvas-wrapper">
        ...
        <button type="button" class="sidebar-toggle left-toggle mobile-hide" @click="isLeftCollapsed = !isLeftCollapsed"
          :title="isLeftCollapsed ? 'Expand Blocks' : 'Collapse Blocks'">
          <ChevronRight v-if="isLeftCollapsed" :size="16" />
          <ChevronLeft v-else :size="16" />
        </button>

        <button type="button" class="sidebar-toggle right-toggle mobile-hide"
        ...
        <main class="builder-canvas" :class="{ 'is-dragging-mode': isDragging }" ref="canvasRef"
          ...
          <div class="canvas-content-limiter" :class="[templateClass, themeClass]"
            ...
            <TransitionGroup tag="div" name="section-list" v-else class="sections-list sections-grid"
              :class="{ 'has-divider': theme.show_divider }" ref="listRef">
              <div v-for="section in gridSections" :key="section.id" :data-id="section.id"
                class="section-wrapper drag-item" :style="dashboardThemeVars" :class="{
                ...
                <SectionBlock :section="section" :is-selected="selectedSectionId === section.id"
                  :theme-class="themeClass" :template-class="templateClass" :theme-vars="themeVars"
                  :is-half-split="section.column_span === 'half'"
                  :class="{ 'block-highlight': highlightedBlocks.has(section.id) }" @select="selectSection(section.id)"
                  @delete="deleteSection(section.id)" @toggle-visibility="store.toggleSectionVisibility(section.id)"
                  @toggle-column-span="store.toggleSectionColumnSpan(section.id)"
                  @move-up="moveSection(section.id, 'up')" @move-down="moveSection(section.id, 'down')"
      ...
      <aside class="props-panel" :class="{ collapsed: isRightCollapsed }" :style="{
        ...
        <div class="props-inner-scroll" ref="propsScrollRef">
          <div class="props-header">
            ...
            <button type="button" class="mobile-close-sidebar @768:flex" @click="isRightCollapsed = true">
              <X :size="18" />
            </button>
          </div>
          ...
          <div class="props-section">
            ...
            <div class="theme-toggle">
              <button type="button" :class="['toggle-btn', { active: theme.mode === 'dark' }]"
                @click="updateTheme({ mode: 'dark', bg_color: '#000000' })">
                <Moon :size="14" /> Dark
              </button>
              <button type="button" :class="['toggle-btn', { active: theme.mode === 'light' }]"
          ...
          <div class="props-section">
            ...
            <div class="color-palette" style="
              ...
              margin-bottom: 12px;
            ">
              <div v-for="(color, idx) in currentColors" :key="idx" class="color-swatch" :style="{ background: color }"
                :class="{ active: theme.primary_color === color }" :title="`Color ${idx + 1}`" @click="
            ...
            <div style="
              ...
              margin-top: 8px;
            ">
              <input type="color" class="form-input" style="height: 36px; padding: 2px" :value="theme.primary_color"
                @input="
                  updateTheme({
              ...
              <input type="color" class="form-input" style="height: 36px; padding: 2px" :value="theme.secondary_color"
                @input="
                  updateTheme({
          ...
          <div class="props-section">
            ...
            <input type="color" class="form-input" style="height: 40px; padding: 2px" v-model="theme.bg_color"
              @change="updateTheme({ bg_color: theme.bg_color })" />
          </div>

          <div class="props-section">
            <div class="props-label">Image Border Color</div>
            <input type="color" class="form-input" style="height: 40px; padding: 2px" v-model="theme.border_color"
              @change="updateTheme({ border_color: theme.border_color })" />
          </div>

          <div class="props-section">
            <div class="props-label">Font</div>
            <select class="form-input form-select" v-model="theme.font" @change="updateTheme({ font: theme.font })">
          ...
          <div class="ai-panel">
            ...
            <template v-if="!aiLoading && !aiResult">
              <!-- Magic Fill Button -->
              <button type="button" class="btn btn-primary btn-sm btn-icon-text magic-fill-btn magic-fill-btn-inline"
                style="width: 100%; margin-bottom: 12px" :disabled="!isHeroComplete" @click="aiMagicFill">
              ...
              <button type="button" class="btn btn-secondary btn-sm btn-icon-text"
                style="width: 100%; margin-bottom: 8px" @click="aiScoreResume">
                <BarChart2 :size="14" /> Score My Resume
              </button>
              <button type="button" class="btn btn-secondary btn-sm btn-icon-text" style="width: 100%"
              ...
              <template v-if="isJson(aiResult) && lastAIAction === 'score'">
                <div class="ai-result-wrap">
                  ...
                  <div style="display: flex; gap: 8px; margin-top: 12px;">
                    <button type="button" @click="aiResult = null" class="btn btn-outline btn-xs"
                ...
                <Transition name="ai-preview">
                  <div class="ai-improve-preview">
                    ...
                    <div class="aip-actions">
                      <button type="button" @click="aiResult = null; lastAIAction = null" class="btn btn-secondary">
                        Discard
                      </button>
                      <button type="button" @click="applyAIResult" class="btn btn-primary aip-apply-btn">
          ...
          <div v-if="selectedSection" class="section-editor" ref="editorRef">
            ...
            <div class="section-toolbar">
              <div class="toolbar-group">
                <button type="button" class="toolbar-btn" @click="moveSection(selectedSection.id, 'up')"
                  title="Move Up">
                  <ChevronUp :size="16" />
                </button>
                <button type="button" class="toolbar-btn" @click="moveSection(selectedSection.id, 'down')"
                  title="Move Down">
                  <ChevronDown :size="16" />
                </button>
              </div>
              ...
              <div class="toolbar-group">
                <button type="button" class="toolbar-btn" @click="store.toggleSectionColumnSpan(selectedSection.id)"
                ...
                <button type="button" class="toolbar-btn" @click="handleDuplicate(selectedSection)" title="Duplicate">
                  <Copy :size="16" />
                </button>
              </div>
              ...
              <button type="button" class="toolbar-btn btn-danger-soft" @click="deleteSection(selectedSection.id)"
                title="Delete">
                <Trash2 :size="16" />
              </button>
            </div>
            ...
            <div v-if="selectedSection.column_span === 'half' && theme.template !== 'firstjobber'" class="props-section"
              style="background: color-mix(in srgb, var(--primary) 5%, transparent); padding: 12px; border-radius: 12px; border: 1px dashed var(--border); margin-bottom: 20px;">
              <label class="toggle-row" style="cursor: pointer;">
                ...
                <div class="toggle-switch">
                  <input type="checkbox" v-model="theme.show_divider"
            ...
            <SectionEditor :section="selectedSection" :template="store.activePortfolio?.theme?.template || 'classic'"
              :is-hero-complete="isHeroComplete" @update="handleSectionUpdate" @magic-fill="aiMagicFill" />

            <!-- Quick access BG color for section -->
            <div v-if="selectedSection" class="props-section"
              ...
              <div style="display: flex; align-items: center; gap: 8px">
                <input type="color" class="form-input" style="width: 44px; height: 36px; padding: 2px; cursor: pointer"
                  :value="selectedSection.data?.section_bg_color || ''"
                  @input="handleSectionBGUpdate(($event.target as HTMLInputElement).value)" />
                <input type="text" class="form-input" style="flex: 1; font-size: 12px" placeholder="Default (Inherit)"
                  :value="selectedSection.data?.section_bg_color || ''"
                  @input="handleSectionBGUpdate(($event.target as HTMLInputElement).value)" />
                <button type="button" v-if="selectedSection.data?.section_bg_color" class="btn btn-secondary btn-sm"
    ...
    <nav v-if="store.activePortfolio" class="mobile-bottom-nav">
      <button type="button" class="mob-nav-btn" :class="{ active: !isLeftCollapsed }" @click="openSidebar('left')">
        <Blocks :size="22" />
        <span>Blocks</span>
      </button>
      <button type="button" class="mob-nav-btn" :class="{ active: store.activePortfolio?.is_published }"
        @click="previewPortfolio" :disabled="!store.activePortfolio?.is_published">
        <Eye :size="22" />
        <span>Preview</span>
      </button>
      <button type="button" class="mob-nav-btn" :class="{ active: !isRightCollapsed && !selectedSectionId }"
        @click="openSidebar('right')">
        <Palette :size="22" />
        <span>Design</span>
      </button>
      <button type="button" v-if="selectedSectionId" class="mob-nav-btn pulse-hint"
        :class="{ active: !isRightCollapsed && !!selectedSectionId }" @click="scrollToEditor">
        <Pencil :size="22" />
        <span>Edit</span>
      </button>
...
function initSortable() {
  ...
  Sortable.create(el, {
    ...
    scroll: true,
    scrollSensitivity: 1, // Approx 1% threshold as requested
    scrollSpeed: 10,
    bubbleScroll: true,
    onStart: () => {
...
function handleSectionUpdate(newData: any) {
  ...
  const { hide_title, hide_divider, include_in_resume, ...actualData } = newData;

  // Optimistic update for real-time canvas preview
  const sec = store.sections.find((s) => s.id === selectedSectionId.value)
  if (sec) {
    // Only update if actually different to prevent unnecessary re-renders or reactive loops
    if (JSON.stringify(sec.data) !== JSON.stringify(actualData)) {
      sec.data = JSON.parse(JSON.stringify(actualData));
    }
    if (sec.hide_title !== !!hide_title) sec.hide_title = !!hide_title;
    if (sec.hide_divider !== !!hide_divider) sec.hide_divider = !!hide_divider;
    if (sec.include_in_resume !== !!include_in_resume) sec.include_in_resume = !!include_in_resume;
  }

  // Use store's debounced update directly
  store.updateSection(selectedSectionId.value, {
    data: actualData,
...
function deleteSection(id: string) {
  // debouncedSave.cancel() // No longer needed as store.updateSection is not debounced here
    ...
    for (const sec of fillable) {
      try {
        ...
        if (secRef && typeof finalData === 'object' && finalData !== null) {
          ...
          store.updateSection(sec.id, { data: finalData });
          successCount++;
        }
      } catch {
    ...
    if (aiSourceSectionId.value) {
      ...
      resultToApply.forEach((item: any) => {
        ...
        if (sec && item.data) {
          ...
          if (sec2) {
            sec2.data = mergedData;
            store.updateSection(sec.id, { data: mergedData });
          }
        }
      });
    } else {
...
.block-palette {
  ...
  overflow: hidden;
  /* Hide content during resize/collapse */
  display: flex;
  flex-direction: column;
...
.sidebar-toggle {
  ...
  cursor: pointer;
  z-index: 100;
  /* Ensure it stays above sections */
...
@media (max-width: 768px) {
  ...
  .sections-grid.tpl-business>.section-wrapper.span-full {
    background: transparent !important;
    /* Stacked, no need for horizontal gradient */
  }
...
/* Drag Reorder Mini View */
.builder-canvas.is-dragging-mode {
  padding-top: 60px;
  padding-bottom: 300px;
}

.builder-canvas.is-dragging-mode .section-wrapper {
  transform: scale(0.8);
...
.builder-canvas.is-dragging-mode .section-wrapper.sortable-chosen {
...
.builder-canvas.is-dragging-mode .section-wrapper.sortable-ghost {
...
/* Hide toolbar and specific UI elements during drag */
.builder-canvas.is-dragging-mode .section-hover-overlay,
.builder-canvas.is-dragging-mode .block-controls,
.builder-canvas.is-dragging-mode .section-divider-add {
  display: none !important;
}

.builder-canvas.is-dragging-mode .section-label-badge {
  display: flex !important;
  transform: scale(1.5);
  background: var(--primary);
  color: #fff;
...
.sections-grid.has-divider .section-wrapper.span-half.is-right::before {
  ...
  bottom: 10%;
  left: -20px;
  /* Offset to visually span the gap */
  width: 1px;
...
.ai-improve-preview {
  ...
  /* enables @container queries on children */
}
...
.builder-mobile-tabs {
  display: none;
  /* Hidden on all sizes — replaced by bottom nav on mobile */
}
