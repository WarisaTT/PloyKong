...
<template>
  <div class="analytics-page">
    <div class="page-header">
      <RouterLink :to="`/portfolios/${route.params.id}/edit`" class="back-btn" title="Builder">
    ...
    <div v-else-if="summary" class="stats-grid">
      <div class="stat-card">
        <div class="stat-label"><Eye :size="14" class="icon-inline" /> Total Views</div>
      ...
      <div class="stat-card">
        <div class="stat-label"><FileDown :size="14" class="icon-inline" /> PDF Downloads</div>
        <div class="stat-num">{{ summary.pdf_downloads }}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label">
          <Send :size="14" class="icon-inline" />
          Hire Me Clicks
        </div>
        <div class="stat-num">{{ summary.hire_clicks }}</div>
      </div>
      <div class="stat-card">
        <div class="stat-label"><Bot :size="14" class="icon-inline" /> AI Chat Sessions</div>
    ...
    <div v-if="summary?.daily_views?.length" class="chart-card">
      <h3><LineChart :size="18" class="icon-inline" /> Views รายวัน (7 วัน)</h3>
      <div class="bar-chart">
    ...
    <div v-if="summary?.country_stats?.length" class="country-card">
      <h3><Globe :size="18" class="icon-inline" /> ผู้เข้าชมจากประเทศ</h3>
...
import { BarChart2, Eye, FileDown, Bot, LineChart, Globe, ArrowLeft, Send } from 'lucide-vue-next'
...
.country-bar-bg { flex: 1; height: 6px; background: var(--border); border-radius: 3px; overflow: hidden; }
...
.stat-label .icon-inline { margin-right: 6px; }
</style>
