      ...
      <div v-if="step === 3" class="step-content animate-fade-in">
        ...
        <div class="theme-grid">
          <div v-for="t in themes" :key="t.id" class="theme-card" :class="{ selected: form.theme === t.id }" @click="form.theme = t.id" :style="t.style">
            ...
            <span class="theme-name"><component :is="t.icon" :size="14" class="icon-inline" /> {{ t.name }}</span>
          </div>
        </div>
        <div style="display:flex;gap:10px;margin-top:20px">
          <button class="btn btn-secondary" @click="step--">← ย้อนกลับ</button>
          <button class="btn btn-primary btn-icon-text" style="flex:1" @click="createPortfolio" :disabled="creating">
            <template v-if="creating"><Loader2 :size="16" class="spin" /> กำลังสร้าง...</template>
...
import {
  Code, PenTool, Sprout, Send, Sparkles, Zap,
  Moon, Sun, Building2, Waves, UserSearch, PenLine, Palette, Loader2, Rocket
...
const themes = [
  { id: 'neon-dark', name: 'Neon Dark', icon: markRaw(Moon), accent: 'linear-gradient(135deg,#4F46E5,#a855f7)', style: 'background:rgba(5,8,20,0.8);border-color:rgba(79,70,229,0.5)' },
  { id: 'clean-light', name: 'Clean Light', icon: markRaw(Sun), accent: 'linear-gradient(135deg,#6366f1,#06b6d4)', style: 'background:rgba(240,245,255,0.1);border-color:rgba(99,102,241,0.4)' },
  { id: 'tokyo-night', name: 'Tokyo Night', icon: markRaw(Building2), accent: 'linear-gradient(135deg,#7c3aed,#ec4899)', style: 'background:rgba(10,5,25,0.8);border-color:rgba(124,58,237,0.5)' },
  { id: 'ocean-cyber', name: 'Ocean Cyber', icon: markRaw(Waves), accent: 'linear-gradient(135deg,#06b6d4,#10b981)', style: 'background:rgba(5,15,25,0.8);border-color:rgba(6,182,212,0.5)' },
]
...
.new-bg { position:fixed;inset:0;background:radial-gradient(ellipse 60% 50% at 50% 30%,rgba(79,70,229,0.15) 0%,transparent 60%); }
.new-card { position:relative;z-index:1;background:var(--surface);border:1px solid var(--border);border-radius:24px;padding:44px;width:100%;max-width:480px;backdrop-filter:blur(30px); }
...
.role-card { display:flex;flex-direction:column;align-items:center;gap:8px;padding:16px 8px;background:var(--bg2);border:1px solid var(--border);border-radius:12px;cursor:pointer;transition:all 0.15s;text-align:center; }
.role-card:hover { border-color:var(--indigo);background:var(--surface); }
.role-card.selected { border-color:var(--purple);background:var(--surface); }
...
.slug-row { display:flex;align-items:center;background:var(--bg);border:1px solid var(--border);border-radius:10px;overflow:hidden; }
...
.theme-card { padding:16px;border:2px solid var(--border);border-radius:12px;cursor:pointer;transition:all 0.15s; }
.theme-card.selected { border-color:var(--purple); }
.theme-preview-bar { height:4px;border-radius:2px;margin-bottom:10px; }
.theme-name { font-size:13px;font-weight:600; display: inline-flex; align-items: center; gap: 4px; }
