<template>
  <div class="skills-preview">
    <div v-if="!data.items || data.items.length === 0" class="empty-msg">ยังไม่มี skills — คลิก Properties เพื่อเพิ่ม</div>
    <div v-else class="skills-list">
      <div v-for="(skill, i) in data.items.slice(0, 5)" :key="i" class="skill-row">
      ...
      <div v-if="data.items.length > 5" class="more-hint">+{{ data.items.length - 5 }} more skills</div>
    </div>
  </div>
</template>
